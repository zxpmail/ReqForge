#!/usr/bin/env node
/**
 * skills-complete — 15 个 Skill 目录齐，且 validate-skill 通过
 */
...
r.assert(names.length === 15, `expected 15 skills, found ${names.length}: ${names.join(", ")}`);
r.assert(
  names.includes("reqforge-greenkeeper"),
  "expected reqforge-greenkeeper among skills",
);
