 ...
 *   - detect-feedback-signal 在 UserPromptSubmit 下（129838b 从 PreToolUse 移来；它是注入 hook，非 stop gate）
 *
...
// detect-feedback-signal 在 129838b 从 PreToolUse 移到 UserPromptSubmit
// （PreToolUse 每次工具调用都触发；UserPromptSubmit 只在用户提交时触发，语义更准且避免 loop）
const USERPROMPTSUBMIT_ONLY = ["detect-feedback-signal"];
...
for (const rel of [
  ...
  const userpromptsubmitHooks = hookNamesUnder(hooks, "UserPromptSubmit");
  for (const hook of USERPROMPTSUBMIT_ONLY) {
    r.assert(
      userpromptsubmitHooks.includes(hook),
      `${rel}: "${hook}" must be under UserPromptSubmit (moved from PreToolUse in 129838b — was firing on every tool call; it is an injection hook, not a stop gate)`
    );
  }
}
