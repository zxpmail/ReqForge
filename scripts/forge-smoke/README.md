...
```bash
pnpm forge-smoke          # 约 15–40 秒，15/15 为通过（含 test-demo 黄金路径）
...
| 5 | `templates-present.mjs` | 核心输出模板存在且有关键章节 |
| 6 | `agents-complete.mjs` | `full` loadout 引用的 agent 均在 `core/agents/` |
| 7 | `hooks-wired.mjs` | `full` loadout 10 个 hook 在 `core/hooks/` 有实现 |
| 8 | `stop-gate-wired.mjs` | Stop 生命周期钩子接线完整 |
| 9 | `skill-fixtures.mjs` | `tests/skill-fixtures/` 静态探针对照 SKILL 正文 |
...
跳过第 14 项（离线/无 pnpm）：`SKIP_TEST_DEMO_GOLDEN=1 pnpm forge-smoke`
