...
- **`evidence_gates`**（选填，推荐）：证据门管道。配置后替代 L2 自由文本 LLM，走 `EG → C1 → C2` 管道。
  ...
  - **`requirements[].type`**：
    ...
    - `"argument-space"` → 走 C3（确定性，零成本）。执行 `verify_command`（独立 runner）观察 side effect，exit 0 → PASS，exit 1 → REJECT（skill-defect）。**不读 evidence 文本，同义词免疫**——series Part 10 的 argument-space 层。
  ...
  - **安全前提**（`editable-surface.json` enforce）：`verify_command` 在 `.forge/content-verify.json`、verify 脚本在 `.forge/verify/`，均在 readonly 区（`constraints.verify_code: false`）。agent 改不了命令（注入）也改不了脚本。详见 [series Part 10 — argument-space, tested](https://dev.to/zxpmail)（实测 C3 5/5，C1/C2 word-space 与真实脱钩；本地稿，尚未发 DEV.to）。
