...
- **forge-loop**: `pnpm forge-loop` — 下班前跑这条命令，第二天来就完事了。自动检查哪些阶段没做完，逐个执行 dev-builder，检测遗漏，修 bug，跑测试，循环直到全部通过。支持 `--all`（全自动）、`--run`（自动开始）、`--serve`（启动网页）、`--url`（网页测试）、`--max`（最多几轮）、`--skip-plan/--skip-ui/--skip-test`（跳过某些检查）、`--reset`（重置）。
