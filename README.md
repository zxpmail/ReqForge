...
- **forge-loop**: `pnpm forge-loop [<N>|--all]` — unified Phase development loop. Orchestrates: dev-builder dispatch (--run) → delivery checklist ⇔ git diff → UI file verification → Playwright tests → pnpm test → auto-create missing files → fix-brief.md for remaining issues. Supports `--all` (auto-detect & execute all incomplete phases), `--serve` (dev server), `--url` (Playwright), `--skip-plan/ui/test`, `--max`, `--reset`. The complete "execute → detect → fix → test → re-check" cycle in one command.
