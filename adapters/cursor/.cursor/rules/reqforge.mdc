...
[General Rules]
    ...
    - **Machine Gates** (enforced by hooks, not by prompt): (1) **Spec-Before-Code Gate** — no `Product-Spec.md` → block `Write`/`Edit` under `src/`, `app/`, `lib/`, `packages/` (via `hallucination-gate` → `spec-before-code-gate.mjs`). (2) **Hallucination Gate** — dependency not found, path wrong, config field missing → fail immediately. (3) **Sloppiness Gate** — no test, no lint, no type check, no verification evidence → block completion. (4) **Overstepping Gate** — spec/schema/convention violation, scope creep, unauthorized changes → reject and flag. Rules that can be codified as lint/test/schema/hook/CI MUST be codified — natural language alone is not enforcement.
