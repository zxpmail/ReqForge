...
### Changed
- **forge-verify: STALE evidence detection implemented (was README-only).** `evidenceFileMeta()` 记录 mtime 已有，但从未与预读 mtime 比对。新增：(1) EG 管道前捕获预读 mtime；(2) 验证结束后重 stat 并比较，不一致时设 `evidence_file.stale: true` + 顶层 `trace.stale: true`。修复 pre-existing bug：`trace` 在 `const` 定义前被 early return 引用 → ReferenceError。README 更新描述。 (`bf96455+`)
- **forge-verify: C2→C1 feedback loop 闭环.** pattern_suggestions 此前仅输出到 stdout/trace，不持久化。新增：(1) 写入 `.forge/verify-pattern-suggestions.json`；(2) `--apply-suggestions` 标志将建议自动合并到 `.forge/content-verify.json`（去重、仅添加真正新的 pattern）。README 新增反馈环文档。 (`bf96455+`)
