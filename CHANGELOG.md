...
### Changed
- **forge-verify: STALE evidence detection implemented (was README-only).** `evidenceFileMeta()` 记录 mtime 已有，但从未与预读 mtime 比对。新增：(1) EG 管道前捕获预读 mtime；(2) 验证结束后重 stat 并比较，不一致时设 `evidence_file.stale: true` + 顶层 `trace.stale: true`。修复 pre-existing bug：`trace` 在 `const` 定义前被 early return 引用 → ReferenceError。README 更新描述。 (`5dc6165`)
- **forge-verify: C2→C1 feedback loop 闭环.** pattern_suggestions 此前仅输出到 stdout/trace，不持久化。新增：(1) 写入 `.forge/verify-pattern-suggestions.json`；(2) `--apply-suggestions` 标志将建议自动合并到 `.forge/content-verify.json`（去重、仅添加真正新的 pattern）。README 新增反馈环文档。 (`5dc6165`)
- **forge-verify: 新增集成测试 + export layeredVerify.** `test-verify-integration.mjs`（13 检查点）验证 trace evidence_files 结构、FILE_NOT_FOUND 路径、无 evidence_gates 路径。导出 `layeredVerify` 供测试 import。 (`5dc6165+`)
- **package.json: 注册全部 forge-verify 测试脚本.** 补充 `test-evidence-gate`、`test-c2c1-feedback`、`test-dfv2-failure-class`。新增 `test-forge-verify-all` 一键全跑。 (`5dc6165+`)
