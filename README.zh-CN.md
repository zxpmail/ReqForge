...
> **和 [OpenSpec](https://github.com/Fission-AI/OpenSpec) 有何不同？** OpenSpec 擅长**存量项目里的一次变更**（CLI + `changes/` + 斜杠命令）。**ReqForge** 覆盖**全流程**，并增加 Harness 能力（钩子、并行审查、反馈进化）；存量增量走 `/change-manager`（思路对齐 OpenSpec）。[详细对照 →](core/docs/openspec-comparison.md)
